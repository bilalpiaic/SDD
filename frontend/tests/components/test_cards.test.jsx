...
let Card, SummaryCard, ListCard, ChartCard;
let COMPONENTS_AVAILABLE = true;
try {
  ({ Card } = require('../../src/components/ui/Card'));
  ({ SummaryCard } = require('../../src/components/cards/SummaryCard'));
  ({ ListCard } = require('../../src/components/cards/ListCard'));
  ({ ChartCard } = require('../../src/components/cards/ChartCard'));
} catch (error) {
  COMPONENTS_AVAILABLE = false;
}
const d = COMPONENTS_AVAILABLE ? describe : describe.skip;

d('Card Base Component', () => {
  const mockCardProps = {
    id: 'card_001',
