...
class AccountsService:
    ...
    async def list(self, *, org_id: Optional[str] = None, **filters: Any) -> MCPResponse:
        ...
        return await self.rpc.call("Accounts.GetAccounts", filters, auth=auth)

    async def get(self, account_id: str, *, org_id: Optional[str] = None) -> MCPResponse:
        auth = MCPAuthContext(xero_organization_id=org_id)
        return await self.rpc.call("Accounts.GetAccount", {"account_id": account_id}, auth=auth)
