...
class ProjectsService:
    ...
    async def list(self, *, org_id: Optional[str] = None, **filters: Any) -> MCPResponse:
        ...
        return await self.rpc.call("Projects.GetProjects", filters, auth=auth)

    async def get(self, project_id: str, *, org_id: Optional[str] = None) -> MCPResponse:
        auth = MCPAuthContext(xero_organization_id=org_id)
        return await self.rpc.call("Projects.GetProject", {"project_id": project_id}, auth=auth)
