...
class AssetsService:
    ...
    async def list(self, *, org_id: Optional[str] = None, **filters: Any) -> MCPResponse:
        ...
        return await self.rpc.call("Assets.GetAssets", filters, auth=auth)

    async def get(self, asset_id: str, *, org_id: Optional[str] = None) -> MCPResponse:
        auth = MCPAuthContext(xero_organization_id=org_id)
        return await self.rpc.call("Assets.GetAsset", {"asset_id": asset_id}, auth=auth)
