...
class ContactsService:
    ...
    async def list(self, *, org_id: Optional[str] = None, **filters: Any) -> MCPResponse:
        ...
        return await self.rpc.call("Contacts.GetContacts", filters, auth=auth)

    async def get(self, contact_id: str, *, org_id: Optional[str] = None) -> MCPResponse:
        auth = MCPAuthContext(xero_organization_id=org_id)
        return await self.rpc.call("Contacts.GetContact", {"contact_id": contact_id}, auth=auth)
